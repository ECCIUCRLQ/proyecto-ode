/*
  ODEcam PCI Driver code

  Modified by:	David Sánchez
			    Oscar Quesada
			    Esteban Porras

  This driver enables communication between ODEcam device
  and a possible end user program.
...
/* Constants */
/***************************************************************************************************************/
#define PCI_CAPTURE_DRIVER "odecam_driver"                  // name of PCI driver
#define PCI_CAPTURE_DRIVER_CHR_DEV "odecam_chr_dev"         // name of character device driver
...
/* This driver supports device with Vendor ID = 0x104C
    also remember that ODEcam's Product ID = 0x0DEC */
static struct pci_device_id pci_capture_driver_table[] = {
    {PCI_DEVICE(0x104C, 0x0DEC)},
    {0,0}
};
...
/* Write random data to test the driver */
void write_sample_data(struct pci_dev *pdev)
{
    struct pci_driver_internal_data *pci_capture_data;
    int data_to_write = 0xCAFEC0C0; // A random value
    uint32_t register_offset = 0x14; // For now we write to 'cmd' register

    pci_capture_data = (struct pci_driver_internal_data *) pci_get_drvdata(pci_dev);

    if (!pci_capture_data) {    // if the data struct is null, abort
        return;
    }
...
*/
void writeToOffset(uint32_t value, uint32_t offset) {
    struct pci_driver_internal_data *pci_capture_data;

    pci_capture_data = (struct pci_driver_internal_data *) pci_get_drvdata(pci_dev);
    iowrite32(value, pci_capture_data->hwmem + offset);
}

/*
    Function to power the device ON or OFF, newstate must be 1 or 0 respectively, returns 1 if successful or 0 if failed. 
*/
int powerDevice(uint32_t newState)
{
    //Check valid parameter, must be 0 or 1
    if(newState < 0 || newState > 1){ return 0; }

    writeToOffset(newState, 0x18);  // 0x18 is the offset for state register
    return 1;
}

/*
    Function to tell the device which filters are being requested. 
*/
int setFilterOptions(uint32_t vintage, uint32_t blur, uint32_t negative, uint32_t blacknWhite)
{
    //Check valid parameter, must be 0 or 1
    if( vintage < 0  || vintage > 1  || blur < 0 || blur > 1 ||
        negative < 0 || negative > 1 || blacknWhite < 0 || blacknWhite > 1 ){ return 0; }

    writeToOffset(vintage,      0x1C);  // 0x1C is vintage filter offset
    writeToOffset(blur,         0x20);  // 0x20 is blur filter offset
    writeToOffset(negative,     0x24);  // 0x24 is negative filter offset
    writeToOffset(blacknWhite,  0x28);  // 0x28 is blacknWhite filter offset
